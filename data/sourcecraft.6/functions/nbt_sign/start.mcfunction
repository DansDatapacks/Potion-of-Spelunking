...
execute if entity @e[type=minecraft:marker,tag=sourcecraft.nbt_sign] run function sourcecraft.6:nbt_sign/end

#create marker that will be where the nbt sign is
execute if predicate sourcecraft.6:in_overworld run summon minecraft:marker ~ 319 ~ {Tags:["sourcecraft.nbt_sign","global.ignore"]}
execute unless predicate sourcecraft.6:in_overworld run summon minecraft:marker ~ 255 ~ {Tags:["sourcecraft.nbt_sign","global.ignore"]}
